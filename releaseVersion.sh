...
CURRENT_VERSION=$(/usr/libexec/PlistBuddy LNPopupUIExample/LNPopupUIExample/Info.plist -c "Print CFBundleShortVersionString")
...
/usr/libexec/PlistBuddy LNPopupUIExample/LNPopupUIExample/Info.plist -c "Set CFBundleShortVersionString $NEXT_VERSION" -c "Set CFBundleVersion 1"
sed -i '' "s|// LNPopupUI:[0-9.]*|// LNPopupUI:$NEXT_VERSION|g" Package.swift
...
gh release create --repo LeoNatan/LNPopupUI "$NEXT_VERSION" --title "$NEXT_VERSION" --notes-file "${RELEASE_NOTES_FILE}"
